...
template <class T>
class Contour : public Surface<BasicColorShader>
{
public:
	Contour(Grid<T>& grid)
		: Surface<BasicColorShader>(new BasicColorShader()),
          m_grid(grid),
          m_isoValue(0.2f),
          m_prevIsoValue(-1.0f)
    {
...
private:
    void marchingSquares(float isoValue, std::vector<float>& data, glm::vec3* color = nullptr)
    {
	...
	void updateSurface(const float& totalTime, const float& frameTime, const Camera& camera, const Light& light)
	{
        ...
        {
			marchingSquares(m_isoValue, m_vertices, &glm::vec3(0, 0, 1.0f));
